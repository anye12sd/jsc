    ...
    >
      <el-form ref="form" :model="form" label-width="80px">
        <el-form-item label="查询时间" v-if="sql_type == 1">
          <el-col :span="11">
            <el-date-picker type="date" :default-value="appTime" placeholder="选择开始日期" v-model="form.date1" style="width: 100%;"
                            :picker-options="pickerOptionsEnd"></el-date-picker>
          </el-col>
          <el-col class="line" :span="2">-</el-col>
          <el-col :span="11">
            <el-date-picker type="date" :default-value="appTime" placeholder="选择结束日期" v-model="form.date2" style="width: 100%;"
                            :picker-options="pickerOptionsEnd"></el-date-picker>
          </el-col>
        </el-form-item>
        <el-form-item
...
<!--          <input type="file" id="file">-->
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" :loading="dialogbtn" @click="submitForm('form')">确 定</el-button>
      </span>
    </el-dialog>
...
<script>
import { getlist, appuserlist, getAppTime, postFormJr, postFormWg } from "@/api/list.js";
      ...
      this.$refs[formName].validate((valid) => {
        ...
        if (valid) {
          ...
          if(this.sql_type == 1){
            ...
            if(this.form.date2 - this.form.date1 >= 366*24*60*60*1000){
              this.$message({
                message: "时间间隔必须在一年之内",
                type: "error",
              });
            ...
            if((this.modular_type == 2 || this.modular_type == 3) && this.form.file.length == 0){
              this.$message({
                message: "请上传excel",
                type: "error",
              });
            ...
            if((this.modular_type == 2 || this.modular_type == 3) && (this.form.file[0].name.split(".")[1] !== 'xlsx' && this.form.file[0].name.split(".")[1] !== 'xls')){
              this.$message({
    ...
    toPostFormJr(){
      ...
      postFormJr(formData).then((res) => {
        ...
        if(res.status == 200){
          ...
          downloadA.download = decodeURIComponent(res.headers['content-disposition'].split(';')[1].split('filename=')[1]);
          downloadA.click()
